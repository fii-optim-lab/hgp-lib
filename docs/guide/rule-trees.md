...
Operators (`And`, `Or`) combine subrules, and literals (`Literal`) test a single feature.
...
`Rule` defines `__slots__` for its four fields (`subrules`, `parent`, `value`, `negated`).
...
The default `And` and `Or` separate literal children from operator children before combining them.
...
The `hgp_lib.rules` package reads `HGP_LOW_MEMORY` at import time and binds `And` and `Or` to the chosen backend.
