...
A boolean rule is a tree of [`And`](api/rules.md#hgp_lib.rules.operators.And) and [`Or`](api/rules.md#hgp_lib.rules.operators.Or) operators over literals, where each literal tests one binarized feature and may be negated.
...
It optimizes the whole rule against the fitness function, evaluates many candidates in parallel, and builds [`And`](api/rules.md#hgp_lib.rules.operators.And) and [`Or`](api/rules.md#hgp_lib.rules.operators.Or) freely in one expression.
...
Hierarchical GP arranges the search as a tree of populations rather than a single flat one.[@albert2026geneabac]
A root population works on the full data, while child populations work on subsets of it.
Each child gets a fraction of the parent's features, or a fraction of its instances, or both.
This is bagging applied across the hierarchy: feature subsets push children to find local attribute correlations, and instance subsets make them robust to noise.

Genetic material flows upward: a parent builds larger rules by recombining the building blocks evolved by its children.
Children are rewarded not only for their own score, but also for how much they contribute to the parent's best rules, which keeps them producing useful fragments.
